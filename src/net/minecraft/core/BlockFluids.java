...
public abstract class BlockFluids extends Block {
    ...
    private Vec3D func_298_c(IBlockAccess iblockaccess, int i, int j, int k) {
        ...
        for (int i1 = 0; i1 < 4; i1++) {
            ...
            if (i2 < 0) {
                if (iblockaccess.getBlockMaterial(j1, k1, l1).func_218_c()) {
                    continue;
                }
        ...
        if (iblockaccess.getBlockMetadata(i, j, k) >= 8) {
            boolean flag = false;
            if (flag || isSideInsideCoordinate(iblockaccess, i, j, k - 1, 2)) {
                flag = true;
            }
            if (flag || isSideInsideCoordinate(iblockaccess, i, j, k + 1, 3)) {
                flag = true;
            }
            if (flag || isSideInsideCoordinate(iblockaccess, i - 1, j, k, 4)) {
                flag = true;
            }
            if (flag || isSideInsideCoordinate(iblockaccess, i + 1, j, k, 5)) {
                flag = true;
            }
            if (flag || isSideInsideCoordinate(iblockaccess, i, j + 1, k - 1, 2)) {
                flag = true;
            }
            if (flag || isSideInsideCoordinate(iblockaccess, i, j + 1, k + 1, 3)) {
                flag = true;
            }
            if (flag || isSideInsideCoordinate(iblockaccess, i - 1, j + 1, k, 4)) {
                flag = true;
            }
            if (flag || isSideInsideCoordinate(iblockaccess, i + 1, j + 1, k, 5)) {
                flag = true;
            }
            if (flag) {
    ...
    private void func_302_i(World world, int i, int j, int k) {
